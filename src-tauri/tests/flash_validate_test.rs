use cvicubemx_lib::flash::{
    default_partitions, validate_partition_layout, validate_partitions, FlashPartition,
    DEFAULT_FLASH_SIZE_KB,
};
...
fn test_flash_partition_validation_integration() {
    ...
    // 3. 空标签校验（期望失败）
    let empty_label = vec![FlashPartition {
        partition_number: 2,
        label: "".into(),
        size: 1024,
        size_string: "1M".into(),
    ...
    // 4. 总大小溢出校验
    let overflow = vec![FlashPartition {
        partition_number: 2,
        label: "BOOT".into(),
        size: DEFAULT_FLASH_SIZE_KB + 1024,
        size_string: "TooBig".into(),
        file: "".into(),
        mountpoint: "".into(),
        type_field: "".into(),
        enabled: true,
