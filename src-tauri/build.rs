...
fn main() {
 ...
 // 带 key 的二进制（否则 api_key为空，请求会被 API以 HTTP400 "缺少授权参数" 拒绝），
 ...
 // Windows exe 的图标由 Tauri 在构建期从 icons/icon.ico 嵌入资源。
 // 图标源或生成后的 ico变化时强制重新运行 build.rs，避免 `tauri build --no-bundle`
 //复用旧的 resource.lib，导致 cvicubemx.exe仍显示旧图标。
...
/// 极简 .env解析：忽略空行与 `#` 注释，按第一个 `=` 分割，去除值两端引号。
