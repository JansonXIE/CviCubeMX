...
const QStringList ClockConfigWidget::CLK_RAW_AXI_SUB_NODES = {
    "clk_oenc", "clk_lvds1_vip", "clk_lvds0_vip", "clk_raw_vip", "clk_disp_vip"
};
...
const QStringList ClockConfigWidget::CLK_SYS_DISP_SUB_NODES = {
    // 根据 clk_summary，sys_disp 下只有 vo_mac_vip，但 UI 中我们保留 disp_vip 作为占位
    "clk_vo_mac_vip"
};
...
const QStringList ClockConfigWidget::CLK_FAB_100M_SUB_NODES = {
    "clk_apb_gpio", "clk_apb_wdt", "clk_apb_vcsys", "clk_apb_jpeg", "clk_apb_ve",
...
const QStringList ClockConfigWidget::CLK_APB_I2C_SUB_NODES = {
    "clk_apb_i2c4", "clk_apb_i2c3", "clk_apb_i2c2", "clk_apb_i2c1", "clk_apb_i2c0"
};

const QStringList ClockConfigWidget::CLK_APB_VCSYS_SUB_NODES = {
    "clk_apb_jpeg", "clk_apb_ve"
};

const QStringList ClockConfigWidget::CLK_X2P_SUB_NODES = {
    "clk_2de_vip", "clk_csi2_rx_vip", "clk_csi1_rx_vip", "clk_csi0_rx_vip", "clk_dsi_mac_vip"
};
...
const QStringList ClockConfigWidget::CLK_RTC_SYS_SUB_NODES = {
    ...
    "clk_rtc_sys_apb_mbox", "clk_rtc_sys_apb_ictrl", "clk_rtc_sys_apb_uart",
    "clk_rtc_sys_apb_timer", "clk_rtc_sys_fab_sram", "clk_rtc_sys_rtc2ap_slv",
    "clk_rtc_sys_hs2rtc_mst", "clk_rtc_sys_mcu"
};
...
const QStringList ClockConfigWidget::CLK_VIP_SYS_2_SUB_NODES = {
    "clk_ldc_vip", "clk_csi_mac1_vip"
};
...
const QStringList ClockConfigWidget::CLK_KEYSCAN_XCLK_SUB_NODES = {
    "clk_keyscan"
};

const QStringList ClockConfigWidget::CLK_WGN_XCLK_SUB_NODES = {
    "clk_wgn"
};
    ...
    , m_isPanning(false)
{
    ...
    setupClkAPBI2CSubNodes();  // 新增：设置clk_apb_i2c子节点区域
    setupClkAPBVCSYSSubNodes();  // 新增：设置clk_apb_vcsys子节点区域
    setupClkX2PSubNodes();  // 新增：设置clk_x2p子节点区域
    ...
    setupClkVIPSYS2SubNodes();  // 新增：设置clk_vip_sys_2子节点区域
    setupClkVIPSYS3SubNodes();  // 新增：设置clk_vip_sys_3子节点区域
    setupClkSPISubNodes();  // 新增：设置clk_spi子节点区域
    setupClkKeyscanXclkSubNodes();  // 新增：设置clk_keyscan_xclk子节点区域
    setupClkWgnXclkSubNodes();  // 新增：设置clk_wgn_xclk子节点区域
...
void ClockConfigWidget::setupClk1MSubNodes()
{
    ...
    for (const QString& nodeName : CLK_1M_SUB_NODES) {
        ...
        // 计算频率：clk_1M的频率 / 分频器
        double clk1MFreq = OSC_FREQUENCY / 250;  // 100kHz = 0.1MHz
...
void ClockConfigWidget::setupClkCam1PLLSubNodes()
{
    ...
    for (const QString& nodeName : CLK_CAM1PLL_SUB_NODES) {
        ...
        double clkCam1PLLFreq = 1600.0; // MHz
        if (m_outputs.contains("clk_cam1pll")) {
            clkCam1PLLFreq = m_outputs["clk_cam1pll"].frequency;
        }
...
void ClockConfigWidget::setupClkCam0PLLSubNodes()
{
    ...
    for (const QString& nodeName : CLK_CAM0PLL_SUB_NODES) {
        ...
        // 从配置中获取正确的频率
        double clkCam0PLLFreq = 1300.0; // MHz (25MHz * 52)
        if (m_outputs.contains("clk_cam0pll")) {
            clkCam0PLLFreq = m_outputs["clk_cam0pll"].frequency;
        }
...
void ClockConfigWidget::setupClkDispPLLSubNodes()
{
    ...
    for (const QString& nodeName : CLK_DISPPLL_SUB_NODES) {
        ...
        // 根据测得频率近似设置默认分频值（保持原 UI 行为并贴近 clk_summary）
        if (nodeName == "clk_cam2_vip") {
            subNode.divider = 32;      // 1200/32≈37.5MHz（与 37.125MHz 接近）
        } else if (nodeName == "clk_cam1_vip") {
            subNode.divider = 44;      // 1200/44≈27.27MHz（与 27.0MHz 接近）
        } else if (nodeName == "clk_sys_disp") {
            subNode.divider = 8;       // 1200/8=150MHz（与 148.5MHz 接近）
        }

        // 计算频率：clk_disppll的频率 / 分频器
        // 从配置中获取正确的频率
        double clkDispPLLFreq = 1188.0; // MHz（更贴近 clk_summary 的 1187.999988）
...
void ClockConfigWidget::setupClkFPLLSubNodes()
{
    ...
    // 将clk_fab_100M与clk_scan_100M的配置同步到m_pllConfigs中，供其子节点/显示使用
    if (m_clkFPLLSubNodes.contains("clk_fab_100M")) {
        ...
        fab100MConfig.inputFreq = m_pllConfigs.contains("clk_fpll") ? m_pllConfigs["clk_fpll"].outputFreq : 1000.0;
        fab100MConfig.multiplier = 1;
        fab100MConfig.outputFreq = m_clkFPLLSubNodes["clk_fab_100M"].frequency; // 应为 100MHz
        m_pllConfigs["clk_fab_100M"] = fab100MConfig;
    }
    if (m_clkFPLLSubNodes.contains("clk_scan_100M")) {
        PLLConfig scan100MConfig;
        scan100MConfig.name = "clk_scan_100M";
        scan100MConfig.enabled = true;
        scan100MConfig.inputFreq = m_pllConfigs.contains("clk_fpll") ? m_pllConfigs["clk_fpll"].outputFreq : 1000.0;
        scan100MConfig.multiplier = 1;
        scan100MConfig.outputFreq = m_clkFPLLSubNodes["clk_scan_100M"].frequency; // ~19.6MHz，保留供 UI 使用
...
void ClockConfigWidget::setupClkMPLLSubNodes()
{
    ...
    if (m_clkMPLLSubNodes.contains("clk_rtc_sys")) {
        ...
        rtcSysConfig.name = "clk_rtc_sys";
        rtcSysConfig.enabled = true;
        rtcSysConfig.inputFreq = 1200.0; // clk_mpll频率
        rtcSysConfig.multiplier = 1;
        rtcSysConfig.outputFreq = m_clkMPLLSubNodes["clk_rtc_sys"].frequency;
        m_pllConfigs["clk_rtc_sys"] = rtcSysConfig;
    }
...
void ClockConfigWidget::setupClkFAB100MSubNodes()
{
    ...
    m_clkFAB100MSubNodeWidget->setFixedWidth(150); // 宽度固定
    ...
    for (const QString& nodeName : CLK_FAB_100M_SUB_NODES) {
        ...
        if (clkFAB100MFreq <= 0 && m_clkFPLLSubNodes.contains("clk_fab_100M")) {
            clkFAB100MFreq = m_clkFPLLSubNodes["clk_fab_100M"].frequency;
        }
    ...
    // 按子项数量动态调整高度（每行控件高度约 40-50，加上标题与边距）
    int per = 48;
    int header = 40;
    int minH = header + per * itemCount + 20; // 额外留白
    if (minH < 120) minH = 120;
    m_clkFAB100MSubNodeWidget->setMinimumHeight(minH);
}
...
void ClockConfigWidget::setupClkAPBI2CSubNodes()
{
    // 创建clk_apb_i2c子节点区域
    m_clkAPBI2CSubNodeWidget = new QWidget();
    m_clkAPBI2CSubNodeWidget->setFixedSize(150, 400); // 设置固定尺寸，5个子节点
    m_clkAPBI2CSubNodeWidget->setStyleSheet(
        "QWidget { "
        "background-color: #e8f0ff; "
        "border: 2px solid #007bff; "
        "border-radius: 8px; "
    ...
    m_clkAPBI2CSubNodeLayout = new QVBoxLayout(m_clkAPBI2CSubNodeWidget);
    m_clkAPBI2CSubNodeLayout->setContentsMargins(10, 10, 10, 10);
    m_clkAPBI2CSubNodeLayout->setSpacing(5);

    // 添加标题
    QLabel* subNodeTitle = new QLabel("clk_apb_i2c子节点");
    subNodeTitle->setStyleSheet("font-size: 14px; font-weight: bold; color: #004085; text-align: center;");
    subNodeTitle->setAlignment(Qt::AlignCenter);
    m_clkAPBI2CSubNodeLayout->addWidget(subNodeTitle);

    // 为每个clk_apb_i2c子节点创建显示界面
    for (const QString& nodeName : CLK_APB_I2C_SUB_NODES) {
        createClkAPBI2CSubNodeWidget(nodeName, m_clkAPBI2CSubNodeWidget);

        // 初始化clk_apb_i2c子节点配置
        ClockOutput subNode;
        subNode.name = nodeName;
        subNode.source = "clk_apb_i2c";
        subNode.divider = 1;  // 默认分频为1
        subNode.enabled = true;

        // 计算频率：clk_apb_i2c的频率 / 分频器
        double clkApbI2CFreq = 100.0;  // 默认100MHz，从clk_i2c->clk_apb_i2c继承
        if (m_clkI2CSubNodes.contains("clk_apb_i2c")) {
            clkApbI2CFreq = m_clkI2CSubNodes["clk_apb_i2c"].frequency;
        }
        subNode.frequency = clkApbI2CFreq / subNode.divider;

        m_clkAPBI2CSubNodes[nodeName] = subNode;
    }

    m_clkAPBI2CSubNodeLayout->addStretch();

    // 设置父widget为m_flowWidget并使用绝对定位
    m_clkAPBI2CSubNodeWidget->setParent(m_flowWidget);
    // 默认位置将在initializeModulePositions中设置
}

void ClockConfigWidget::setupClkAPBVCSYSSubNodes()
{
    // 创建clk_apb_vcsys子节点区域
    m_clkAPBVCSYSSubNodeWidget = new QWidget();
    m_clkAPBVCSYSSubNodeWidget->setFixedSize(150, 180); // 设置固定尺寸，2个子节点
    m_clkAPBVCSYSSubNodeWidget->setStyleSheet(
        "QWidget { "
        "background-color: #e8f0ff; "
        "border: 2px solid #007bff; "
        "border-radius: 8px; "
    ...
    m_clkAPBVCSYSSubNodeLayout = new QVBoxLayout(m_clkAPBVCSYSSubNodeWidget);
    m_clkAPBVCSYSSubNodeLayout->setContentsMargins(10, 10, 10, 10);
    m_clkAPBVCSYSSubNodeLayout->setSpacing(5);

    // 添加标题
    QLabel* subNodeTitle = new QLabel("clk_apb_vcsys子节点");
    subNodeTitle->setStyleSheet("font-size: 14px; font-weight: bold; color: #004085; text-align: center;");
    subNodeTitle->setAlignment(Qt::AlignCenter);
    m_clkAPBVCSYSSubNodeLayout->addWidget(subNodeTitle);

    // 为每个clk_apb_vcsys子节点创建显示界面
    for (const QString& nodeName : CLK_APB_VCSYS_SUB_NODES) {
        createClkAPBVCSYSSubNodeWidget(nodeName, m_clkAPBVCSYSSubNodeWidget);

        // 初始化clk_apb_vcsys子节点配置
        ClockOutput subNode;
        subNode.name = nodeName;
        subNode.source = "clk_apb_vcsys";
        subNode.divider = 1;  // 默认分频为1
        subNode.enabled = true;

        // 计算频率：clk_apb_vcsys的频率 / 分频器
        double clkApbVcsysFreq = 100.0;  // 默认100MHz，从clk_fab_100M->clk_apb_vcsys继承
        if (m_clkFAB100MSubNodes.contains("clk_apb_vcsys")) {
            clkApbVcsysFreq = m_clkFAB100MSubNodes["clk_apb_vcsys"].frequency;
        }
        subNode.frequency = clkApbVcsysFreq / subNode.divider;

        m_clkAPBVCSYSSubNodes[nodeName] = subNode;
    }

    m_clkAPBVCSYSSubNodeLayout->addStretch();

    // 设置父widget为m_flowWidget并使用绝对定位
    m_clkAPBVCSYSSubNodeWidget->setParent(m_flowWidget);
    // 默认位置将在initializeModulePositions中设置
}

void ClockConfigWidget::setupClkX2PSubNodes()
{
    // 创建clk_x2p子节点区域
    m_clkX2PSubNodeWidget = new QWidget();
    m_clkX2PSubNodeWidget->setFixedSize(150, 420); // 设置固定尺寸，5个子节点
    m_clkX2PSubNodeWidget->setStyleSheet(
        "QWidget { "
        "background-color: #e8f0ff; "
        "border: 2px solid #007bff; "
        "border-radius: 8px; "
    ...
    m_clkX2PSubNodeLayout = new QVBoxLayout(m_clkX2PSubNodeWidget);
    m_clkX2PSubNodeLayout->setContentsMargins(10, 10, 10, 10);
    m_clkX2PSubNodeLayout->setSpacing(5);

    // 添加标题
    QLabel* subNodeTitle = new QLabel("clk_x2p子节点");
    subNodeTitle->setStyleSheet("font-size: 14px; font-weight: bold; color: #004085; text-align: center;");
    subNodeTitle->setAlignment(Qt::AlignCenter);
    m_clkX2PSubNodeLayout->addWidget(subNodeTitle);

    // 为每个clk_x2p子节点创建显示界面
    for (const QString& nodeName : CLK_X2P_SUB_NODES) {
        createClkX2PSubNodeWidget(nodeName, m_clkX2PSubNodeWidget);

        // 初始化clk_x2p子节点配置
        ClockOutput subNode;
        subNode.name = nodeName;
        subNode.source = "clk_x2p";
        subNode.divider = 1;  // 默认分频为1
        subNode.enabled = true;

        // 计算频率：clk_x2p的频率 / 分频器
        double clkX2PFreq = 100.0;  // 默认100MHz，从clk_fab_100M->clk_x2p继承
        if (m_clkFAB100MSubNodes.contains("clk_x2p")) {
            clkX2PFreq = m_clkFAB100MSubNodes["clk_x2p"].frequency;
        }
        subNode.frequency = clkX2PFreq / subNode.divider;

        m_clkX2PSubNodes[nodeName] = subNode;
    }

    m_clkX2PSubNodeLayout->addStretch();

    // 设置父widget为m_flowWidget并使用绝对定位
    m_clkX2PSubNodeWidget->setParent(m_flowWidget);
    // 默认位置将在initializeModulePositions中设置
}
...
{
    // 创建clk_rtc_sys子节点区域
    m_clkRTCSYSSubNodeWidget = new QWidget();
    m_clkRTCSYSSubNodeWidget->setFixedSize(150, 1400); // 设置固定尺寸，14个子节点
    m_clkRTCSYSSubNodeWidget->setStyleSheet(
        "QWidget { "
        "background-color: #ffffff; "
        "border: 2px solid #17a2b8; "
        "border-radius: 8px; "
    ...
    m_clkRTCSYSSubNodeLayout = new QVBoxLayout(m_clkRTCSYSSubNodeWidget);
    m_clkRTCSYSSubNodeLayout->setContentsMargins(10, 10, 10, 10);
    m_clkRTCSYSSubNodeLayout->setSpacing(5);

    // 添加标题
    QLabel* subNodeTitle = new QLabel("clk_rtc_sys子节点");
    subNodeTitle->setStyleSheet("font-size: 14px; font-weight: bold; color: #117a8b; text-align: center;");
    subNodeTitle->setAlignment(Qt::AlignCenter);
    m_clkRTCSYSSubNodeLayout->addWidget(subNodeTitle);
    // 为每个clk_rtc_sys子节点创建显示界面
    for (const QString& nodeName : CLK_RTC_SYS_SUB_NODES) {
        createClkRTCSYSSubNodeWidget(nodeName, m_clkRTCSYSSubNodeWidget);

        // 初始化clk_rtc_sys子节点配置
        ClockOutput subNode;
        subNode.name = nodeName;
        subNode.source = "clk_rtc_sys";
        subNode.enabled = true;
        // 根据子节点设置不同的默认分频值
        if (nodeName == "clk_rtc_sys_apb_saradc1") {
            subNode.divider = 1;  // clk_rtc_sys_apb_saradc1默认分频系数是1
        } else if (nodeName == "clk_rtc_sys_apb_wdt") {
            subNode.divider = 1;  // clk_rtc_sys_apb_wdt默认分频系数是1
        } else if (nodeName == "clk_rtc_sys_apb_saradc") {
            subNode.divider = 1;  // clk_rtc_sys_apb_saradc默认分频系数是1
        } else if (nodeName == "clk_rtc_sys_apb_i2c") {
            subNode.divider = 1;  // clk_rtc_sys_apb_i2c默认分频系数是1
        } else if (nodeName == "clk_rtc_sys_apb_osc") {
            subNode.divider = 1;  // clk_rtc_sys_apb_osc默认分频系数是1
        } else if (nodeName == "clk_rtc_sys_apb_gpio") {
            subNode.divider = 1;  // clk_rtc_sys_apb_gpio默认分频系数是1
        } else if (nodeName == "clk_rtc_sys_apb_mbox") {
            subNode.divider = 1;  // clk_rtc_sys_apb_mbox默认分频系数是1
        } else if (nodeName == "clk_rtc_sys_apb_ictrl") {
            subNode.divider = 1;  // clk_rtc_sys_apb_ictrl默认分频系数是1
        } else if (nodeName == "clk_rtc_sys_apb_uart") {
            subNode.divider = 1;  // clk_rtc_sys_apb_uart默认分频系数是1
        } else if (nodeName == "clk_rtc_sys_apb_timer") {
            subNode.divider = 1;  // clk_rtc_sys_apb_timer默认分频系数是1
        } else if (nodeName == "clk_rtc_sys_fab_sram") {
            subNode.divider = 1;  // clk_rtc_sys_fab_sram默认分频系数是1
        } else if (nodeName == "clk_rtc_sys_rtc2ap_slv") {
            subNode.divider = 1;  // clk_rtc_sys_rtc2ap_slv默认分频系数是1
        } else if (nodeName == "clk_rtc_sys_hs2rtc_mst") {
            subNode.divider = 1;  // clk_rtc_sys_hs2rtc_mst默认分频系数是1
        } else if (nodeName == "clk_rtc_sys_mcu") {
            subNode.divider = 1;  // clk_rtc_sys_mcu默认分频系数是1
        }

        // 计算频率：clk_rtc_sys的频率 / 分频器
        // 从clk_rtc_sys的配置中获取正确的频率
        double clkRTCSYSFreq = 300.0; // 默认300MHz
        if (m_pllConfigs.contains("clk_rtc_sys")) {
            clkRTCSYSFreq = m_pllConfigs["clk_rtc_sys"].outputFreq;
        }
        subNode.frequency = clkRTCSYSFreq / subNode.divider;

        m_clkRTCSYSSubNodes[nodeName] = subNode;
    }

    m_clkRTCSYSSubNodeLayout->addStretch();
...
void ClockConfigWidget::setupClkVIPSYS2SubNodes()
{
    // 创建clk_vip_sys_2子节点区域
    m_clkVIPSYS2SubNodeWidget = new QWidget();
    m_clkVIPSYS2SubNodeWidget->setFixedSize(150, 200); // 设置固定尺寸，2个子节点
    m_clkVIPSYS2SubNodeWidget->setStyleSheet(
        "QWidget { "
    ...
    m_clkVIPSYS2SubNodeLayout = new QVBoxLayout(m_clkVIPSYS2SubNodeWidget);
    m_clkVIPSYS2SubNodeLayout->setContentsMargins(10, 10, 10, 10);
    m_clkVIPSYS2SubNodeLayout->setSpacing(5);

    // 添加标题
    QLabel* subNodeTitle = new QLabel("clk_vip_sys_2子节点");
    subNodeTitle->setStyleSheet("font-size: 14px; font-weight: bold; color: #009900; text-align: center;");
    subNodeTitle->setAlignment(Qt::AlignCenter);
    m_clkVIPSYS2SubNodeLayout->addWidget(subNodeTitle);

    // 为每个clk_vip_sys_2子节点创建显示界面
    for (const QString& nodeName : CLK_VIP_SYS_2_SUB_NODES) {
        createClkVIPSYS2SubNodeWidget(nodeName, m_clkVIPSYS2SubNodeWidget);

        // 初始化clk_vip_sys_2子节点配置
        ClockOutput subNode;
        subNode.name = nodeName;
        subNode.source = "clk_vip_sys_2";
        subNode.enabled = true;
        subNode.divider = 1;  // 默认分频系数是1

        // 计算频率：clk_vip_sys_2的频率 / 分频器
        // 从clk_cam1pll子节点中获取clk_vip_sys_2的频率
        double clkVIPSYS2Freq = 1600.0; // 默认1600MHz
        if (m_clkCam1PLLSubNodes.contains("clk_vip_sys_2")) {
            clkVIPSYS2Freq = m_clkCam1PLLSubNodes["clk_vip_sys_2"].frequency;
        }
        subNode.frequency = clkVIPSYS2Freq / subNode.divider;

        m_clkVIPSYS2SubNodes[nodeName] = subNode;
    }

    m_clkVIPSYS2SubNodeLayout->addStretch();

    // 设置父widget为m_flowWidget并使用绝对定位
    m_clkVIPSYS2SubNodeWidget->setParent(m_flowWidget);
    // 默认位置将在initializeModulePositions中设置
}
...
void ClockConfigWidget::setupClkKeyscanXclkSubNodes()
{
    // 创建clk_keyscan_xclk子节点区域
    m_clkKeyscanXclkSubNodeWidget = new QWidget();
    m_clkKeyscanXclkSubNodeWidget->setFixedSize(150, 150); // 设置固定尺寸，1个子节点
    m_clkKeyscanXclkSubNodeWidget->setStyleSheet(
        "QWidget { "
    ...
    m_clkKeyscanXclkSubNodeLayout = new QVBoxLayout(m_clkKeyscanXclkSubNodeWidget);
    m_clkKeyscanXclkSubNodeLayout->setContentsMargins(10, 10, 10, 10);
    m_clkKeyscanXclkSubNodeLayout->setSpacing(5);

    // 添加标题
    QLabel* subNodeTitle = new QLabel("clk_keyscan_xclk子节点");
    subNodeTitle->setStyleSheet("font-size: 14px; font-weight: bold; color: #cc0000; text-align: center;");
    subNodeTitle->setAlignment(Qt::AlignCenter);
    m_clkKeyscanXclkSubNodeLayout->addWidget(subNodeTitle);

    // 为每个clk_keyscan_xclk子节点创建显示界面
    for (const QString& nodeName : CLK_KEYSCAN_XCLK_SUB_NODES) {
        createClkKeyscanXclkSubNodeWidget(nodeName, m_clkKeyscanXclkSubNodeWidget);

        // 初始化clk_keyscan_xclk子节点配置
        ClockOutput subNode;
        subNode.name = nodeName;
        subNode.source = "clk_keyscan_xclk";
        subNode.enabled = true;
        subNode.divider = 1;  // 默认分频系数是1

        // 计算频率：clk_keyscan_xclk的频率 / 分频器
        double clkKeyscanXclkFreq = OSC_FREQUENCY;  // 从OSC获取
        if (m_outputs.contains("clk_keyscan_xclk")) {
            clkKeyscanXclkFreq = m_outputs["clk_keyscan_xclk"].frequency;
        }
        subNode.frequency = clkKeyscanXclkFreq / subNode.divider;

        m_clkKeyscanXclkSubNodes[nodeName] = subNode;
    }

    m_clkKeyscanXclkSubNodeLayout->addStretch();

    // 设置父widget为m_flowWidget并使用绝对定位
    m_clkKeyscanXclkSubNodeWidget->setParent(m_flowWidget);
    // 默认位置将在initializeModulePositions中设置
}

void ClockConfigWidget::setupClkWgnXclkSubNodes()
{
    // 创建clk_wgn_xclk子节点区域
    m_clkWgnXclkSubNodeWidget = new QWidget();
    m_clkWgnXclkSubNodeWidget->setFixedSize(150, 150); // 设置固定尺寸，1个子节点
    m_clkWgnXclkSubNodeWidget->setStyleSheet(
        "QWidget { "
    ...
    m_clkWgnXclkSubNodeLayout = new QVBoxLayout(m_clkWgnXclkSubNodeWidget);
    m_clkWgnXclkSubNodeLayout->setContentsMargins(10, 10, 10, 10);
    m_clkWgnXclkSubNodeLayout->setSpacing(5);

    // 添加标题
    QLabel* subNodeTitle = new QLabel("clk_wgn_xclk子节点");
    subNodeTitle->setStyleSheet("font-size: 14px; font-weight: bold; color: #0066cc; text-align: center;");
    subNodeTitle->setAlignment(Qt::AlignCenter);
    m_clkWgnXclkSubNodeLayout->addWidget(subNodeTitle);

    // 为每个clk_wgn_xclk子节点创建显示界面
    for (const QString& nodeName : CLK_WGN_XCLK_SUB_NODES) {
        createClkWgnXclkSubNodeWidget(nodeName, m_clkWgnXclkSubNodeWidget);

        // 初始化clk_wgn_xclk子节点配置
        ClockOutput subNode;
        subNode.name = nodeName;
        subNode.source = "clk_wgn_xclk";
        subNode.enabled = true;
        subNode.divider = 1;  // 默认分频系数是1

        // 计算频率：clk_wgn_xclk的频率 / 分频器
        double clkWgnXclkFreq = OSC_FREQUENCY;  // 从OSC获取
        if (m_outputs.contains("clk_wgn_xclk")) {
            clkWgnXclkFreq = m_outputs["clk_wgn_xclk"].frequency;
        }
        subNode.frequency = clkWgnXclkFreq / subNode.divider;

        m_clkWgnXclkSubNodes[nodeName] = subNode;
    }

    m_clkWgnXclkSubNodeLayout->addStretch();

    // 设置父widget为m_flowWidget并使用绝对定位
    m_clkWgnXclkSubNodeWidget->setParent(m_flowWidget);
    // 默认位置将在initializeModulePositions中设置
}
...
void ClockConfigWidget::createClkFAB100MSubNodeWidget(const QString& nodeName, QWidget* parent)
{
    ...
    double calculatedFreq = clkFAB100MFreq / defaultDivider;
    QString freqText = QString("%1 MHz").arg(calculatedFreq, 0, 'f', 1);
...
void ClockConfigWidget::createClkAPBI2CSubNodeWidget(const QString& nodeName, QWidget* parent)
{
    QWidget* subNodeWidget = new QWidget();
    subNodeWidget->setStyleSheet(
        "QWidget { "
    ...
    // 频率显示
    double clkApbI2CFreq = 100.0;  // 默认100MHz，从clk_apb_i2c继承
    double calculatedFreq = clkApbI2CFreq / 1;  // 默认分频为1
    QString freqText;
    if (calculatedFreq >= 1.0) {
        freqText = QString("%1 MHz").arg(calculatedFreq, 0, 'f', 3);
    } else {
    ...
    // 存储控件引用
    m_clkAPBI2CSubNodeWidgets[nodeName] = subNodeWidget;
    m_clkAPBI2CSubNodeFreqLabels[nodeName] = freqLabel;
    m_clkAPBI2CSubNodeDividerBoxes[nodeName] = divBox;

    m_clkAPBI2CSubNodeLayout->addWidget(subNodeWidget);

    // 连接分频器变化信号
    connect(divBox, QOverload<int>::of(&QSpinBox::valueChanged),
            [this, nodeName](int value) {
                onClkAPBI2CSubNodeDividerChanged(nodeName, value);
            });
}

void ClockConfigWidget::createClkAPBVCSYSSubNodeWidget(const QString& nodeName, QWidget* parent)
{
    QWidget* subNodeWidget = new QWidget();
    subNodeWidget->setStyleSheet(
        "QWidget { "
    ...
    // 频率显示
    double clkApbVcsysFreq = 100.0;  // 默认100MHz，从clk_apb_vcsys继承
    double calculatedFreq = clkApbVcsysFreq / 1;  // 默认分频为1
    QString freqText;
    if (calculatedFreq >= 1.0) {
        freqText = QString("%1 MHz").arg(calculatedFreq, 0, 'f', 3);
    } else {
    ...
    // 存储控件引用
    m_clkAPBVCSYSSubNodeWidgets[nodeName] = subNodeWidget;
    m_clkAPBVCSYSSubNodeFreqLabels[nodeName] = freqLabel;
    m_clkAPBVCSYSSubNodeDividerBoxes[nodeName] = divBox;

    m_clkAPBVCSYSSubNodeLayout->addWidget(subNodeWidget);

    // 连接分频器变化信号
    connect(divBox, QOverload<int>::of(&QSpinBox::valueChanged),
            [this, nodeName](int value) {
                onClkAPBVCSYSSubNodeDividerChanged(nodeName, value);
            });
}

void ClockConfigWidget::createClkX2PSubNodeWidget(const QString& nodeName, QWidget* parent)
{
    QWidget* subNodeWidget = new QWidget();
    subNodeWidget->setStyleSheet(
        "QWidget { "
    ...
    // 频率显示
    double clkX2PFreq = 100.0;  // 默认100MHz，从clk_x2p继承
    double calculatedFreq = clkX2PFreq / 1;  // 默认分频为1
    QString freqText;
    if (calculatedFreq >= 1.0) {
        freqText = QString("%1 MHz").arg(calculatedFreq, 0, 'f', 3);
    } else {
    ...
    // 存储控件引用
    m_clkX2PSubNodeWidgets[nodeName] = subNodeWidget;
    m_clkX2PSubNodeFreqLabels[nodeName] = freqLabel;
    m_clkX2PSubNodeDividerBoxes[nodeName] = divBox;

    m_clkX2PSubNodeLayout->addWidget(subNodeWidget);

    // 连接分频器变化信号
    connect(divBox, QOverload<int>::of(&QSpinBox::valueChanged),
            [this, nodeName](int value) {
                onClkX2PSubNodeDividerChanged(nodeName, value);
            });
}
...
void ClockConfigWidget::createClkRTCSYSSubNodeWidget(const QString& nodeName, QWidget* parent)
{
    ...
    // 根据子节点设置不同的默认分频值
    if (nodeName == "clk_rtc_sys_apb_saradc1") {
        divBox->setValue(1);  // clk_rtc_sys_apb_saradc1默认分频系数是1
    } else if (nodeName == "clk_rtc_sys_apb_wdt") {
        divBox->setValue(1);  // clk_rtc_sys_apb_wdt默认分频系数是1
    } else if (nodeName == "clk_rtc_sys_apb_saradc") {
        divBox->setValue(1);  // clk_rtc_sys_apb_saradc默认分频系数是1
    } else if (nodeName == "clk_rtc_sys_apb_i2c") {
        divBox->setValue(1);  // clk_rtc_sys_apb_i2c默认分频系数是1
    } else if (nodeName == "clk_rtc_sys_apb_osc") {
        divBox->setValue(1);  // clk_rtc_sys_apb_osc默认分频系数是1
    } else if (nodeName == "clk_rtc_sys_apb_gpio") {
        divBox->setValue(1);  // clk_rtc_sys_apb_gpio默认分频系数是1
    } else if (nodeName == "clk_rtc_sys_apb_mbox") {
        divBox->setValue(1);  // clk_rtc_sys_apb_mbox默认分频系数是1
    } else if (nodeName == "clk_rtc_sys_apb_ictrl") {
        divBox->setValue(1);  // clk_rtc_sys_apb_ictrl默认分频系数是1
    } else if (nodeName == "clk_rtc_sys_apb_uart") {
        divBox->setValue(1);  // clk_rtc_sys_apb_uart默认分频系数是1
    } else if (nodeName == "clk_rtc_sys_apb_timer") {
        divBox->setValue(1);  // clk_rtc_sys_apb_timer默认分频系数是1
    } else if (nodeName == "clk_rtc_sys_fab_sram") {
        divBox->setValue(1);  // clk_rtc_sys_fab_sram默认分频系数是1
    } else if (nodeName == "clk_rtc_sys_rtc2ap_slv") {
        divBox->setValue(1);  // clk_rtc_sys_rtc2ap_slv默认分频系数是1
    } else if (nodeName == "clk_rtc_sys_hs2rtc_mst") {
        divBox->setValue(1);  // clk_rtc_sys_hs2rtc_mst默认分频系数是1
    } else if (nodeName == "clk_rtc_sys_mcu") {
    ...
    // 频率显示
    double clkRTCSYSFreq = 300.0;  // 假设clk_rtc_sys = 300MHz
    double calculatedFreq;
    if (nodeName == "clk_rtc_sys_apb_saradc1") {
        calculatedFreq = clkRTCSYSFreq / 1;  // clk_rtc_sys_apb_saradc1默认分频系数是1
    } else if (nodeName == "clk_rtc_sys_apb_wdt") {
        calculatedFreq = clkRTCSYSFreq / 1;  // clk_rtc_sys_apb_wdt默认分频系数是1
    } else if (nodeName == "clk_rtc_sys_apb_saradc") {
        calculatedFreq = clkRTCSYSFreq / 1;  // clk_rtc_sys_apb_saradc默认分频系数是1
    } else if (nodeName == "clk_rtc_sys_apb_i2c") {
        calculatedFreq = clkRTCSYSFreq / 1;  // clk_rtc_sys_apb_i2c默认分频系数是1
    } else if (nodeName == "clk_rtc_sys_apb_osc") {
        calculatedFreq = clkRTCSYSFreq / 1;  // clk_rtc_sys_apb_osc默认分频系数是1
    } else if (nodeName == "clk_rtc_sys_apb_gpio") {
        calculatedFreq = clkRTCSYSFreq / 1;  // clk_rtc_sys_apb_gpio默认分频系数是1
    } else if (nodeName == "clk_rtc_sys_apb_mbox") {
        calculatedFreq = clkRTCSYSFreq / 1;  // clk_rtc_sys_apb_mbox默认分频系数是1
    } else if (nodeName == "clk_rtc_sys_apb_ictrl") {
        calculatedFreq = clkRTCSYSFreq / 1;  // clk_rtc_sys_apb_ictrl默认分频系数是1
    } else if (nodeName == "clk_rtc_sys_apb_uart") {
        calculatedFreq = clkRTCSYSFreq / 1;  // clk_rtc_sys_apb_uart默认分频系数是1
    } else if (nodeName == "clk_rtc_sys_apb_timer") {
        calculatedFreq = clkRTCSYSFreq / 1;  // clk_rtc_sys_apb_timer默认分频系数是1
    } else if (nodeName == "clk_rtc_sys_fab_sram") {
        calculatedFreq = clkRTCSYSFreq / 1;  // clk_rtc_sys_fab_sram默认分频系数是1
    } else if (nodeName == "clk_rtc_sys_rtc2ap_slv") {
        calculatedFreq = clkRTCSYSFreq / 1;  // clk_rtc_sys_rtc2ap_slv默认分频系数是1
    } else if (nodeName == "clk_rtc_sys_hs2rtc_mst") {
        calculatedFreq = clkRTCSYSFreq / 1;  // clk_rtc_sys_hs2rtc_mst默认分频系数是1
    } else if (nodeName == "clk_rtc_sys_mcu") {
        calculatedFreq = clkRTCSYSFreq / 1;  // clk_rtc_sys_mcu默认分频系数是1
    }
    QString freqText = QString("%1 MHz").arg(calculatedFreq, 0, 'f', 1);
    ...
    // 存储控件引用
    m_clkRTCSYSSubNodeWidgets[nodeName] = subNodeWidget;
    m_clkRTCSYSSubNodeFreqLabels[nodeName] = freqLabel;
    m_clkRTCSYSSubNodeDividerBoxes[nodeName] = divBox;

    m_clkRTCSYSSubNodeLayout->addWidget(subNodeWidget);

    // 连接分频器变化信号
    connect(divBox, QOverload<int>::of(&QSpinBox::valueChanged),
            [this, nodeName](int value) {
                onClkRTCSYSSubNodeDividerChanged(nodeName, value);
            });
}
...
void ClockConfigWidget::createClkVIPSYS2SubNodeWidget(const QString& nodeName, QWidget* parent)
{
    QWidget* subNodeWidget = new QWidget();
    subNodeWidget->setStyleSheet(
        "QWidget { "
    ...
    QLabel* nameLabel = new QLabel();
    nameLabel->setText(formatClockTitle(nodeName));
    nameLabel->setStyleSheet("font-weight: bold; color: #009900; font-size: 11px;");
    nameLabel->setAlignment(Qt::AlignCenter);
    nameLabel->setWordWrap(true);
    ...
    QLabel* divLabel = new QLabel("分频：");
    divLabel->setStyleSheet("color: #009900; font-size: 10px; font-weight: bold;");
    ...
    // 频率显示
    double clkVIPSYS2Freq = 400.0;  // 假设clk_vip_sys_2 = 400MHz
    double calculatedFreq = clkVIPSYS2Freq / 1;  // 所有子节点默认分频系数是1
    QString freqText = QString("%1 MHz").arg(calculatedFreq, 0, 'f', 1);
    ...
    // 存储控件引用
    m_clkVIPSYS2SubNodeWidgets[nodeName] = subNodeWidget;
    m_clkVIPSYS2SubNodeFreqLabels[nodeName] = freqLabel;
    m_clkVIPSYS2SubNodeDividerBoxes[nodeName] = divBox;

    m_clkVIPSYS2SubNodeLayout->addWidget(subNodeWidget);

    // 连接分频器变化信号
    connect(divBox, QOverload<int>::of(&QSpinBox::valueChanged),
            [this, nodeName](int value) {
                onClkVIPSYS2SubNodeDividerChanged(nodeName, value);
            });
}
...
void ClockConfigWidget::createClkKeyscanXclkSubNodeWidget(const QString& nodeName, QWidget* parent)
{
    QWidget* subNodeWidget = new QWidget();
    subNodeWidget->setStyleSheet(
        "QWidget { "
    ...
    QLabel* nameLabel = new QLabel();
    nameLabel->setText(formatClockTitle(nodeName));
    nameLabel->setStyleSheet("font-weight: bold; color: #cc0000; font-size: 11px;");
    nameLabel->setAlignment(Qt::AlignCenter);
    nameLabel->setWordWrap(true);
    ...
    QLabel* divLabel = new QLabel("分频：");
    divLabel->setStyleSheet("color: #cc0000; font-size: 10px; font-weight: bold;");
    ...
    // 频率显示
    double clkKeyscanXclkFreq = OSC_FREQUENCY;
    double calculatedFreq = clkKeyscanXclkFreq / 1;
    QString freqText = QString("%1 MHz").arg(calculatedFreq, 0, 'f', 3);
    ...
    // 存储控件引用
    m_clkKeyscanXclkSubNodeWidgets[nodeName] = subNodeWidget;
    m_clkKeyscanXclkSubNodeFreqLabels[nodeName] = freqLabel;
    m_clkKeyscanXclkSubNodeDividerBoxes[nodeName] = divBox;

    m_clkKeyscanXclkSubNodeLayout->addWidget(subNodeWidget);
}

void ClockConfigWidget::createClkWgnXclkSubNodeWidget(const QString& nodeName, QWidget* parent)
{
    QWidget* subNodeWidget = new QWidget();
    subNodeWidget->setStyleSheet(
        "QWidget { "
    ...
    // 频率显示
    double clkWgnXclkFreq = OSC_FREQUENCY;
    double calculatedFreq = clkWgnXclkFreq / 1;
    QString freqText = QString("%1 MHz").arg(calculatedFreq, 0, 'f', 3);
    ...
    // 存储控件引用
    m_clkWgnXclkSubNodeWidgets[nodeName] = subNodeWidget;
    m_clkWgnXclkSubNodeFreqLabels[nodeName] = freqLabel;
    m_clkWgnXclkSubNodeDividerBoxes[nodeName] = divBox;

    m_clkWgnXclkSubNodeLayout->addWidget(subNodeWidget);
}
...
void ClockConfigWidget::initializeModulePositions()
{
    ...
    // 优化后的布局：从左到右分层，垂直对齐，减少连接线交叉
    
    // 第一列：输入源 (最左侧)
    ModulePosition inputPos = {"输入源", 30, 50, 150, 400};
    
    // 第二列：主PLL (锁相环)
    ModulePosition pllPos = {"锁相环", 250, 50, 200, 700};
    ...
    m_modulePositions["clk_vip_sys_2子节点"] = clkVIPSYS2SubPos;
    m_modulePositions["clk_vip_sys_3子节点"] = clkVIPSYS3SubPos;
    m_modulePositions["clk_spi子节点"] = clkSPISubPos;
    m_modulePositions["clk_apb_i2c子节点"] = clkAPBI2CSubPos;
    m_modulePositions["clk_apb_vcsys子节点"] = clkAPBVCSYSSubPos;
    m_modulePositions["clk_x2p子节点"] = clkX2PSubPos;
    m_modulePositions["clk_keyscan_xclk子节点"] = clkKeyscanXclkSubPos;
    m_modulePositions["clk_wgn_xclk子节点"] = clkWgnXclkSubPos;
...
{
    // 在应用位置前，对 clk_fab100m 子节点根据子项数量动态拉伸高度
    if (m_clkFAB100MSubNodeLayout && m_clkFAB100MSubNodeWidget && m_modulePositions.contains("clk_fab100m子节点")) {
        int autoHeight = m_clkFAB100MSubNodeLayout->sizeHint().height() + 20; // 额外留白
        if (autoHeight > m_modulePositions["clk_fab100m子节点"].height) {
            m_modulePositions["clk_fab100m子节点"].height = autoHeight;
        }
    }
    ...
    if (m_modulePositions.contains("clk_rtc_sys子节点") && m_clkRTCSYSSubNodeWidget) {
        ...
        m_clkRTCSYSSubNodeWidget->setFixedSize(pos.width, pos.height);
    }
    ...
    if (m_modulePositions.contains("clk_vip_sys_2子节点") && m_clkVIPSYS2SubNodeWidget) {
        ModulePosition pos = m_modulePositions["clk_vip_sys_2子节点"];
        m_clkVIPSYS2SubNodeWidget->move(pos.x, pos.y);
        m_clkVIPSYS2SubNodeWidget->setFixedSize(pos.width, pos.height);
    }
    ...
    if (m_modulePositions.contains("clk_apb_i2c子节点") && m_clkAPBI2CSubNodeWidget) {
        ModulePosition pos = m_modulePositions["clk_apb_i2c子节点"];
        m_clkAPBI2CSubNodeWidget->move(pos.x, pos.y);
        m_clkAPBI2CSubNodeWidget->setFixedSize(pos.width, pos.height);
    }

    if (m_modulePositions.contains("clk_apb_vcsys子节点") && m_clkAPBVCSYSSubNodeWidget) {
        ModulePosition pos = m_modulePositions["clk_apb_vcsys子节点"];
        m_clkAPBVCSYSSubNodeWidget->move(pos.x, pos.y);
        m_clkAPBVCSYSSubNodeWidget->setFixedSize(pos.width, pos.height);
    }

    if (m_modulePositions.contains("clk_x2p子节点") && m_clkX2PSubNodeWidget) {
        ModulePosition pos = m_modulePositions["clk_x2p子节点"];
        m_clkX2PSubNodeWidget->move(pos.x, pos.y);
        m_clkX2PSubNodeWidget->setFixedSize(pos.width, pos.height);
    }

    if (m_modulePositions.contains("clk_keyscan_xclk子节点") && m_clkKeyscanXclkSubNodeWidget) {
        ModulePosition pos = m_modulePositions["clk_keyscan_xclk子节点"];
        m_clkKeyscanXclkSubNodeWidget->move(pos.x, pos.y);
        m_clkKeyscanXclkSubNodeWidget->setFixedSize(pos.width, pos.height);
    }

    if (m_modulePositions.contains("clk_wgn_xclk子节点") && m_clkWgnXclkSubNodeWidget) {
        ModulePosition pos = m_modulePositions["clk_wgn_xclk子节点"];
        m_clkWgnXclkSubNodeWidget->move(pos.x, pos.y);
        m_clkWgnXclkSubNodeWidget->setFixedSize(pos.width, pos.height);
    }
...
void ClockConfigWidget::ensureFlowCanvasFitsContent()
{
    ...
    allWidgets << m_inputWidget << m_pllWidget << m_subPllWidget << m_outputWidget
               ...
               << m_clkVIPSYS1SubNodeWidget << m_clkVIPSYS2SubNodeWidget << m_clkVIPSYS3SubNodeWidget 
               << m_clkSPISubNodeWidget << m_clkAPBI2CSubNodeWidget << m_clkAPBVCSYSSubNodeWidget 
               << m_clkX2PSubNodeWidget << m_clkKeyscanXclkSubNodeWidget << m_clkWgnXclkSubNodeWidget;
...
void ClockConfigWidget::onClkMPLLSubNodeDividerChanged(const QString& nodeName, int divider)
{
    if (m_clkMPLLSubNodes.contains(nodeName)) {
        ...
        // 如果修改的是clk_rtc_sys，需要同步更新其在m_pllConfigs中的频率，并更新其子节点
        if (nodeName == "clk_rtc_sys") {
            // 同步更新m_pllConfigs中的频率
            if (m_pllConfigs.contains("clk_rtc_sys")) {
                m_pllConfigs["clk_rtc_sys"].outputFreq = m_clkMPLLSubNodes[nodeName].frequency;
            }
...
void ClockConfigWidget::onClkAPBI2CSubNodeDividerChanged(const QString& nodeName, int divider)
{
    if (m_clkAPBI2CSubNodes.contains(nodeName)) {
        m_clkAPBI2CSubNodes[nodeName].divider = divider;
        updateClkAPBI2CSubNodeFrequency(nodeName);
        emit configChanged();
    }
}

void ClockConfigWidget::onClkAPBVCSYSSubNodeDividerChanged(const QString& nodeName, int divider)
{
    if (m_clkAPBVCSYSSubNodes.contains(nodeName)) {
        m_clkAPBVCSYSSubNodes[nodeName].divider = divider;
        updateClkAPBVCSYSSubNodeFrequency(nodeName);
        emit configChanged();
    }
}

void ClockConfigWidget::onClkX2PSubNodeDividerChanged(const QString& nodeName, int divider)
{
    if (m_clkX2PSubNodes.contains(nodeName)) {
        m_clkX2PSubNodes[nodeName].divider = divider;
        updateClkX2PSubNodeFrequency(nodeName);
        emit configChanged();
    }
}

void ClockConfigWidget::onClkRTCSYSSubNodeDividerChanged(const QString& nodeName, int divider)
{
    if (m_clkRTCSYSSubNodes.contains(nodeName)) {
        m_clkRTCSYSSubNodes[nodeName].divider = divider;
...
void ClockConfigWidget::onClkVIPSYS2SubNodeDividerChanged(const QString& nodeName, int divider)
{
    if (m_clkVIPSYS2SubNodes.contains(nodeName)) {
        m_clkVIPSYS2SubNodes[nodeName].divider = divider;
        updateClkVIPSYS2SubNodeFrequency(nodeName);
        emit configChanged();
    }
}
...
void ClockConfigWidget::updatePLLFrequency(const QString& pllName)
{
    ...
    // 若更新的是 FPLL，联动其派生的 fab_100M 与 scan_100M
    if (pllName == "clk_fpll") {
        if (m_clkFPLLSubNodes.contains("clk_fab_100M")) {
            m_pllConfigs["clk_fab_100M"].outputFreq = m_clkFPLLSubNodes["clk_fab_100M"].frequency;
        }
        if (m_clkFPLLSubNodes.contains("clk_scan_100M")) {
            m_pllConfigs["clk_scan_100M"].outputFreq = m_clkFPLLSubNodes["clk_scan_100M"].frequency;
        }
        // 推动 fab_100M 子节点重算
        updateAllClkFAB100MSubNodeFrequencies();
    }
...
void ClockConfigWidget::updateAllClkCam1PLLSubNodeFrequencies()
{
    ...
    // 更新clk_vip_sys_2的所有子节点频率
    updateAllClkVIPSYS2SubNodeFrequencies();
}
...
void ClockConfigWidget::updateAllClkFPLLSubNodeFrequencies()
{
    ...
    // 同步 m_pllConfigs 兜底，避免 fab_100M 初值为 0
    if (m_clkFPLLSubNodes.contains("clk_fab_100M")) {
        m_pllConfigs["clk_fab_100M"].outputFreq = m_clkFPLLSubNodes["clk_fab_100M"].frequency;
    }
...
void ClockConfigWidget::updateClkFAB100MSubNodeFrequency(const QString& nodeName)
{
    ...
    if (m_clkFAB100MSubNodeFreqLabels.contains(nodeName)) {
        QString freqText = QString("%1 MHz").arg(qMax(0.0, subNode.frequency), 0, 'f', 1);
...
void ClockConfigWidget::updateClkAPBI2CSubNodeFrequency(const QString& nodeName)
{
    if (!m_clkAPBI2CSubNodes.contains(nodeName)) return;

    ClockOutput& subNode = m_clkAPBI2CSubNodes[nodeName];

    // 获取clk_apb_i2c的频率（从clk_i2c子节点获取）
    double clkApbI2CFreq = 100.0; // 默认100MHz
    if (m_clkI2CSubNodes.contains("clk_apb_i2c")) {
        clkApbI2CFreq = m_clkI2CSubNodes["clk_apb_i2c"].frequency;
    }

    // clk_apb_i2c子节点频率 = clk_apb_i2c频率 / 分频器
    subNode.frequency = clkApbI2CFreq / subNode.divider;

    // 更新显示
    if (m_clkAPBI2CSubNodeFreqLabels.contains(nodeName)) {
        QString freqText;
        if (subNode.frequency >= 1.0) {
            freqText = QString("%1 MHz").arg(subNode.frequency, 0, 'f', 3);
        } else {
        ...
        m_clkAPBI2CSubNodeFreqLabels[nodeName]->setText(freqText);
    }
}

void ClockConfigWidget::updateAllClkAPBI2CSubNodeFrequencies()
{
    for (const QString& nodeName : CLK_APB_I2C_SUB_NODES) {
        updateClkAPBI2CSubNodeFrequency(nodeName);
    }
...
void ClockConfigWidget::updateClkAPBVCSYSSubNodeFrequency(const QString& nodeName)
{
    if (!m_clkAPBVCSYSSubNodes.contains(nodeName)) return;

    ClockOutput& subNode = m_clkAPBVCSYSSubNodes[nodeName];

    // 获取clk_apb_vcsys的频率（从clk_fab_100M子节点获取）
    double clkApbVcsysFreq = 100.0; // 默认100MHz
    if (m_clkFAB100MSubNodes.contains("clk_apb_vcsys")) {
        clkApbVcsysFreq = m_clkFAB100MSubNodes["clk_apb_vcsys"].frequency;
    }

    // clk_apb_vcsys子节点频率 = clk_apb_vcsys频率 / 分频器
    subNode.frequency = clkApbVcsysFreq / subNode.divider;

    // 更新显示
    if (m_clkAPBVCSYSSubNodeFreqLabels.contains(nodeName)) {
        QString freqText;
        if (subNode.frequency >= 1.0) {
            freqText = QString("%1 MHz").arg(subNode.frequency, 0, 'f', 3);
        } else {
        ...
        m_clkAPBVCSYSSubNodeFreqLabels[nodeName]->setText(freqText);
    }
}

void ClockConfigWidget::updateAllClkAPBVCSYSSubNodeFrequencies()
{
    for (const QString& nodeName : CLK_APB_VCSYS_SUB_NODES) {
        updateClkAPBVCSYSSubNodeFrequency(nodeName);
    }
...
void ClockConfigWidget::updateClkX2PSubNodeFrequency(const QString& nodeName)
{
    if (!m_clkX2PSubNodes.contains(nodeName)) return;

    ClockOutput& subNode = m_clkX2PSubNodes[nodeName];

    // 获取clk_x2p的频率（从clk_fab_100M子节点获取）
    double clkX2PFreq = 100.0; // 默认100MHz
    if (m_clkFAB100MSubNodes.contains("clk_x2p")) {
        clkX2PFreq = m_clkFAB100MSubNodes["clk_x2p"].frequency;
    }

    // clk_x2p子节点频率 = clk_x2p频率 / 分频器
    subNode.frequency = clkX2PFreq / subNode.divider;

    // 更新显示
    if (m_clkX2PSubNodeFreqLabels.contains(nodeName)) {
        QString freqText;
        if (subNode.frequency >= 1.0) {
            freqText = QString("%1 MHz").arg(subNode.frequency, 0, 'f', 3);
        } else {
        ...
        m_clkX2PSubNodeFreqLabels[nodeName]->setText(freqText);
    }
}

void ClockConfigWidget::updateAllClkX2PSubNodeFrequencies()
{
    for (const QString& nodeName : CLK_X2P_SUB_NODES) {
        updateClkX2PSubNodeFrequency(nodeName);
    }
...
void ClockConfigWidget::updateClkRTCSYSSubNodeFrequency(const QString& nodeName)
{
    if (!m_clkRTCSYSSubNodes.contains(nodeName)) return;

    ClockOutput& subNode = m_clkRTCSYSSubNodes[nodeName];
    ...
    // clk_rtc_sys子节点频率 = clk_rtc_sys频率 / 分频器
    subNode.frequency = clkRTCSYSFreq / subNode.divider;

    // 更新显示
    if (m_clkRTCSYSSubNodeFreqLabels.contains(nodeName)) {
        QString freqText = QString("%1 MHz").arg(subNode.frequency, 0, 'f', 1);
        m_clkRTCSYSSubNodeFreqLabels[nodeName]->setText(freqText);
    }
}

void ClockConfigWidget::updateAllClkRTCSYSSubNodeFrequencies()
{
    for (const QString& nodeName : CLK_RTC_SYS_SUB_NODES) {
        updateClkRTCSYSSubNodeFrequency(nodeName);
    }
...
void ClockConfigWidget::updateClkVIPSYS2SubNodeFrequency(const QString& nodeName)
{
    if (!m_clkVIPSYS2SubNodes.contains(nodeName)) return;

    ClockOutput& subNode = m_clkVIPSYS2SubNodes[nodeName];

    // 获取clk_vip_sys_2的频率（来自clk_cam1pll子节点）
    double clkVIPSYS2Freq = 400.0; // 默认400MHz
    if (m_clkCam1PLLSubNodes.contains("clk_vip_sys_2")) {
        clkVIPSYS2Freq = m_clkCam1PLLSubNodes["clk_vip_sys_2"].frequency;
    }

    // clk_vip_sys_2子节点频率 = clk_vip_sys_2频率 / 分频器
    subNode.frequency = clkVIPSYS2Freq / subNode.divider;

    // 更新显示
    if (m_clkVIPSYS2SubNodeFreqLabels.contains(nodeName)) {
        QString freqText = QString("%1 MHz").arg(subNode.frequency, 0, 'f', 1);
        m_clkVIPSYS2SubNodeFreqLabels[nodeName]->setText(freqText);
    }
}

void ClockConfigWidget::updateAllClkVIPSYS2SubNodeFrequencies()
{
    for (const QString& nodeName : CLK_VIP_SYS_2_SUB_NODES) {
        updateClkVIPSYS2SubNodeFrequency(nodeName);
    }
...
void ClockConfigWidget::updateClkKeyscanXclkSubNodeFrequency(const QString& nodeName)
{
    if (!m_clkKeyscanXclkSubNodes.contains(nodeName)) return;

    ClockOutput& subNode = m_clkKeyscanXclkSubNodes[nodeName];

    // 获取clk_keyscan_xclk的频率
    double clkKeyscanXclkFreq = OSC_FREQUENCY;
    if (m_outputs.contains("clk_keyscan_xclk")) {
        clkKeyscanXclkFreq = m_outputs["clk_keyscan_xclk"].frequency;
    }

    // clk_keyscan_xclk子节点频率 = clk_keyscan_xclk频率 / 分频器
    subNode.frequency = clkKeyscanXclkFreq / subNode.divider;

    // 更新显示
    if (m_clkKeyscanXclkSubNodeFreqLabels.contains(nodeName)) {
        QString freqText = QString("%1 MHz").arg(subNode.frequency, 0, 'f', 3);
        m_clkKeyscanXclkSubNodeFreqLabels[nodeName]->setText(freqText);
    }
}

void ClockConfigWidget::updateAllClkKeyscanXclkSubNodeFrequencies()
{
    for (const QString& nodeName : CLK_KEYSCAN_XCLK_SUB_NODES) {
        updateClkKeyscanXclkSubNodeFrequency(nodeName);
    }
}

void ClockConfigWidget::updateClkWgnXclkSubNodeFrequency(const QString& nodeName)
{
    if (!m_clkWgnXclkSubNodes.contains(nodeName)) return;

    ClockOutput& subNode = m_clkWgnXclkSubNodes[nodeName];
    ...
    // clk_wgn_xclk子节点频率 = clk_wgn_xclk频率 / 分频器
    subNode.frequency = clkWgnXclkFreq / subNode.divider;

    // 更新显示
    if (m_clkWgnXclkSubNodeFreqLabels.contains(nodeName)) {
        QString freqText = QString("%1 MHz").arg(subNode.frequency, 0, 'f', 3);
        m_clkWgnXclkSubNodeFreqLabels[nodeName]->setText(freqText);
    }
}

void ClockConfigWidget::updateAllClkWgnXclkSubNodeFrequencies()
{
    for (const QString& nodeName : CLK_WGN_XCLK_SUB_NODES) {
        updateClkWgnXclkSubNodeFrequency(nodeName);
    }
}
...
void ClockConfigWidget::updateFrequencies()
{
    ...
    // 更新所有clk_apb_i2c子节点频率
    for (const QString& nodeName : CLK_APB_I2C_SUB_NODES) {
        updateClkAPBI2CSubNodeFrequency(nodeName);
    }

    // 更新所有clk_apb_vcsys子节点频率
    for (const QString& nodeName : CLK_APB_VCSYS_SUB_NODES) {
        updateClkAPBVCSYSSubNodeFrequency(nodeName);
    }
...
void ClockConfigWidget::drawConnectionLines(QPainter& painter)
{
    ...
    // 绘制clk_cam1pll到其子节点的连接线（从OSC输出区域）
    if (m_clkCam1PLLSubNodeWidget) {
        QPoint cam1PLLPoint = getOutputNodeConnectionPoint("clk_cam1pll");
    ...
    // 绘制clk_cam0pll到其子节点的连接线（从OSC输出区域）
    if (m_clkCam0PLLSubNodeWidget) {
        QPoint cam0PLLPoint = getOutputNodeConnectionPoint("clk_cam0pll");
    ...
    // 绘制clk_apb_i2c到其子节点的连接线
    if (m_clkAPBI2CSubNodeWidget) {
        QPoint apbI2CPoint = getClkAPBI2CConnectionPoint();
        QPoint subNodePoint = getClkAPBI2CSubNodeConnectionPoint();
        if (!subNodePoint.isNull() && !apbI2CPoint.isNull()) {
            // 使用蓝色来表示clk_apb_i2c到子节点的连接
            QColor lineColor = QColor(0, 123, 255);  // 蓝色

            // 计算肘形拐点
            QPoint elbow1, elbow2;
            calculateElbowPoints(apbI2CPoint, subNodePoint, elbow1, elbow2);
            drawElbowArrowLine(painter, apbI2CPoint, elbow1, elbow2, subNodePoint, lineColor);
        }
    }

    // 绘制clk_apb_vcsys到其子节点的连接线
    if (m_clkAPBVCSYSSubNodeWidget) {
        QPoint apbVcsysPoint = getClkAPBVCSYSConnectionPoint();
        QPoint subNodePoint = getClkAPBVCSYSSubNodeConnectionPoint();
        if (!subNodePoint.isNull() && !apbVcsysPoint.isNull()) {
            // 使用蓝色来表示clk_apb_vcsys到子节点的连接
            QColor lineColor = QColor(0, 123, 255);  // 蓝色

            // 计算肘形拐点
            QPoint elbow1, elbow2;
            calculateElbowPoints(apbVcsysPoint, subNodePoint, elbow1, elbow2);
            drawElbowArrowLine(painter, apbVcsysPoint, elbow1, elbow2, subNodePoint, lineColor);
        }
    }

    // 绘制clk_x2p到其子节点的连接线
    if (m_clkX2PSubNodeWidget) {
        QPoint x2pPoint = getClkX2PConnectionPoint();
        QPoint subNodePoint = getClkX2PSubNodeConnectionPoint();
        if (!subNodePoint.isNull() && !x2pPoint.isNull()) {
            // 使用蓝色来表示clk_x2p到子节点的连接
            QColor lineColor = QColor(0, 123, 255);  // 蓝色

            // 计算肘形拐点
            QPoint elbow1, elbow2;
            calculateElbowPoints(x2pPoint, subNodePoint, elbow1, elbow2);
            drawElbowArrowLine(painter, x2pPoint, elbow1, elbow2, subNodePoint, lineColor);
        }
    }
    ...
    // 绘制clk_rtc_sys到其子节点的连接线
    if (m_clkRTCSYSSubNodeWidget) {
        QPoint rtcSysPoint = getClkRTCSYSConnectionPoint();
        QPoint subNodePoint = getClkRTCSYSSubNodeConnectionPoint();
        if (!subNodePoint.isNull() && !rtcSysPoint.isNull()) {
            // 使用深橙色来表示clk_rtc_sys到子节点的连接
            QColor lineColor = QColor(255, 140, 0);  // 深橙色
    ...
    // 绘制clk_vip_sys_2到其子节点的连接线
    if (m_clkVIPSYS2SubNodeWidget) {
        QPoint vipSys2Point = getClkVIPSYS2ConnectionPoint();
        QPoint subNodePoint = getClkVIPSYS2SubNodeConnectionPoint();
        if (!subNodePoint.isNull() && !vipSys2Point.isNull()) {
            // 使用绿色来表示clk_vip_sys_2到子节点的连接
            QColor lineColor = QColor(51, 204, 51);  // 绿色

            // 计算肘形拐点
            QPoint elbow1, elbow2;
            calculateElbowPoints(vipSys2Point, subNodePoint, elbow1, elbow2);
            drawElbowArrowLine(painter, vipSys2Point, elbow1, elbow2, subNodePoint, lineColor);
        }
    }
    ...
    // 绘制clk_keyscan_xclk到其子节点的连接线（从OSC输出区域）
    if (m_clkKeyscanXclkSubNodeWidget) {
        QPoint keyscanXclkPoint = getOutputNodeConnectionPoint("clk_keyscan_xclk");
        QPoint subNodePoint = getClkKeyscanXclkSubNodeConnectionPoint();
        if (!subNodePoint.isNull() && !keyscanXclkPoint.isNull()) {
            // 使用红色来表示clk_keyscan_xclk到子节点的连接
            QColor lineColor = QColor(204, 0, 0);  // 红色

            // 计算肘形拐点
            QPoint elbow1, elbow2;
            calculateElbowPoints(keyscanXclkPoint, subNodePoint, elbow1, elbow2);
            drawElbowArrowLine(painter, keyscanXclkPoint, elbow1, elbow2, subNodePoint, lineColor);
        }
    }

    // 绘制clk_wgn_xclk到其子节点的连接线（从OSC输出区域）
    if (m_clkWgnXclkSubNodeWidget) {
        QPoint wgnXclkPoint = getOutputNodeConnectionPoint("clk_wgn_xclk");
        QPoint subNodePoint = getClkWgnXclkSubNodeConnectionPoint();
        if (!subNodePoint.isNull() && !wgnXclkPoint.isNull()) {
            // 使用蓝色来表示clk_wgn_xclk到子节点的连接
            QColor lineColor = QColor(0, 102, 204);  // 蓝色

            // 计算肘形拐点
            QPoint elbow1, elbow2;
            calculateElbowPoints(wgnXclkPoint, subNodePoint, elbow1, elbow2);
            drawElbowArrowLine(painter, wgnXclkPoint, elbow1, elbow2, subNodePoint, lineColor);
        }
    }
}
...
QPoint ClockConfigWidget::getOutputNodeConnectionPoint(const QString& nodeName) const
{
    if (!m_outputWidget || !m_flowWidget || !m_outputWidgets.contains(nodeName)) {
        return QPoint();
    }

    QWidget* nodeWidget = m_outputWidgets[nodeName];
    if (!nodeWidget) {
        return QPoint();
    }

    // 获取节点widget在其父widget中的位置
    QPoint nodePos = nodeWidget->pos();
    QRect nodeRect = nodeWidget->rect();
    ...
    // 节点连接点位于widget的右侧中央
    return QPoint(
        outputAreaPos.x() + nodePos.x() + nodeRect.width(),
        outputAreaPos.y() + nodePos.y() + nodeRect.height() / 2
    );
}
...
QPoint ClockConfigWidget::getClkAPBI2CConnectionPoint() const
{
    if (!m_clkI2CSubNodeWidget || !m_flowWidget || !m_clkI2CSubNodeWidgets.contains("clk_apb_i2c")) {
        return QPoint();
    }

    QWidget* apbI2CWidget = m_clkI2CSubNodeWidgets["clk_apb_i2c"];
    if (!apbI2CWidget) {
        return QPoint();
    }

    // 获取clk_apb_i2c widget在其父widget中的位置
    QPoint apbI2CPos = apbI2CWidget->pos();
    QRect apbI2CRect = apbI2CWidget->rect();

    // 获取clk_i2c区域在flow widget中的位置
    QPoint i2cAreaPos = m_clkI2CSubNodeWidget->pos();

    // clk_apb_i2c连接点位于widget的右侧中央
    QPoint apbI2CPoint = QPoint(
        i2cAreaPos.x() + apbI2CPos.x() + apbI2CRect.width(),
        i2cAreaPos.y() + apbI2CPos.y() + apbI2CRect.height() / 2
    );

    return apbI2CPoint;
}

QPoint ClockConfigWidget::getClkAPBI2CSubNodeConnectionPoint() const
{
    if (!m_clkAPBI2CSubNodeWidget || !m_flowWidget) {
        return QPoint();
    }

    // 获取子节点widget在其父widget中的位置
    QPoint subNodePos = m_clkAPBI2CSubNodeWidget->pos();
    QRect subNodeRect = m_clkAPBI2CSubNodeWidget->rect();
...
QPoint ClockConfigWidget::getClkAPBVCSYSConnectionPoint() const
{
    if (!m_clkFAB100MSubNodeWidget || !m_flowWidget || !m_clkFAB100MSubNodeWidgets.contains("clk_apb_vcsys")) {
        return QPoint();
    }

    QWidget* apbVcsysWidget = m_clkFAB100MSubNodeWidgets["clk_apb_vcsys"];
    if (!apbVcsysWidget) {
        return QPoint();
    }

    // 获取clk_apb_vcsys widget在其父widget中的位置
    QPoint apbVcsysPos = apbVcsysWidget->pos();
    QRect apbVcsysRect = apbVcsysWidget->rect();

    // 获取clk_fab_100M区域在flow widget中的位置
    QPoint fab100MAreaPos = m_clkFAB100MSubNodeWidget->pos();

    // clk_apb_vcsys连接点位于widget的右侧中央
    QPoint apbVcsysPoint = QPoint(
        fab100MAreaPos.x() + apbVcsysPos.x() + apbVcsysRect.width(),
        fab100MAreaPos.y() + apbVcsysPos.y() + apbVcsysRect.height() / 2
    );

    return apbVcsysPoint;
}

QPoint ClockConfigWidget::getClkAPBVCSYSSubNodeConnectionPoint() const
{
    if (!m_clkAPBVCSYSSubNodeWidget || !m_flowWidget) {
        return QPoint();
    }

    // 获取子节点widget在其父widget中的位置
    QPoint subNodePos = m_clkAPBVCSYSSubNodeWidget->pos();
    QRect subNodeRect = m_clkAPBVCSYSSubNodeWidget->rect();
...
QPoint ClockConfigWidget::getClkX2PConnectionPoint() const
{
    if (!m_clkFAB100MSubNodeWidget || !m_flowWidget || !m_clkFAB100MSubNodeWidgets.contains("clk_x2p")) {
        return QPoint();
    }

    QWidget* x2pWidget = m_clkFAB100MSubNodeWidgets["clk_x2p"];
    if (!x2pWidget) {
        return QPoint();
    }

    // 获取clk_x2p widget在其父widget中的位置
    QPoint x2pPos = x2pWidget->pos();
    QRect x2pRect = x2pWidget->rect();

    // 获取clk_fab_100M区域在flow widget中的位置
    QPoint fab100MAreaPos = m_clkFAB100MSubNodeWidget->pos();

    // clk_x2p连接点位于widget的右侧中央
    QPoint x2pPoint = QPoint(
        fab100MAreaPos.x() + x2pPos.x() + x2pRect.width(),
        fab100MAreaPos.y() + x2pPos.y() + x2pRect.height() / 2
    );

    return x2pPoint;
}

QPoint ClockConfigWidget::getClkX2PSubNodeConnectionPoint() const
{
    if (!m_clkX2PSubNodeWidget || !m_flowWidget) {
        return QPoint();
    }

    // 获取子节点widget在其父widget中的位置
    QPoint subNodePos = m_clkX2PSubNodeWidget->pos();
    QRect subNodeRect = m_clkX2PSubNodeWidget->rect();
...
QPoint ClockConfigWidget::getClkRTCSYSConnectionPoint() const
{
    if (!m_clkMPLLSubNodeWidget || !m_flowWidget || !m_clkMPLLSubNodeWidgets.contains("clk_rtc_sys")) {
        return QPoint();
    }

    QWidget* rtcSysWidget = m_clkMPLLSubNodeWidgets["clk_rtc_sys"];
    if (!rtcSysWidget) {
        return QPoint();
    }

    // 获取clk_rtc_sys widget在其父widget中的位置
    QPoint rtcSysPos = rtcSysWidget->pos();
    QRect rtcSysRect = rtcSysWidget->rect();
    // 获取子PLL区域在flow widget中的位置
    QPoint subPllAreaPos = m_clkMPLLSubNodeWidget->pos();

    // clk_rtc_sys连接点位于widget的右侧中央
    QPoint rtcSysPoint = QPoint(
        subPllAreaPos.x() + rtcSysPos.x() + rtcSysRect.width(),
        subPllAreaPos.y() + rtcSysPos.y() + rtcSysRect.height() / 2
    );

    return rtcSysPoint;
}

QPoint ClockConfigWidget::getClkRTCSYSSubNodeConnectionPoint() const
{
    if (!m_clkRTCSYSSubNodeWidget || !m_flowWidget) {
        return QPoint();
    }

    // 获取子节点widget在其父widget中的位置
    QPoint subNodePos = m_clkRTCSYSSubNodeWidget->pos();
    QRect subNodeRect = m_clkRTCSYSSubNodeWidget->rect();
...
QPoint ClockConfigWidget::getClkVIPSYS2ConnectionPoint() const
{
    if (!m_clkCam1PLLSubNodeWidget || !m_flowWidget || !m_clkCam1PLLSubNodeWidgets.contains("clk_vip_sys_2")) {
        return QPoint();
    }

    QWidget* vipSys2Widget = m_clkCam1PLLSubNodeWidgets["clk_vip_sys_2"];
    if (!vipSys2Widget) {
        return QPoint();
    }

    // 获取clk_vip_sys_2 widget在其父widget中的位置
    QPoint vipSys2Pos = vipSys2Widget->pos();
    QRect vipSys2Rect = vipSys2Widget->rect();
    // 获取cam1pll子节点区域在flow widget中的位置
    QPoint cam1PLLAreaPos = m_clkCam1PLLSubNodeWidget->pos();

    // clk_vip_sys_2连接点位于widget的右侧中央
    QPoint vipSys2Point = QPoint(
        cam1PLLAreaPos.x() + vipSys2Pos.x() + vipSys2Rect.width(),
        cam1PLLAreaPos.y() + vipSys2Pos.y() + vipSys2Rect.height() / 2
    );

    return vipSys2Point;
}

QPoint ClockConfigWidget::getClkVIPSYS2SubNodeConnectionPoint() const
{
    if (!m_clkVIPSYS2SubNodeWidget || !m_flowWidget) {
        return QPoint();
    }

    // 获取子节点widget在其父widget中的位置
    QPoint subNodePos = m_clkVIPSYS2SubNodeWidget->pos();
    QRect subNodeRect = m_clkVIPSYS2SubNodeWidget->rect();
...
QPoint ClockConfigWidget::getClkKeyscanXclkSubNodeConnectionPoint() const
{
    if (!m_clkKeyscanXclkSubNodeWidget || !m_flowWidget) {
        return QPoint();
    }

    // 获取子节点widget在其父widget中的位置
    QPoint subNodePos = m_clkKeyscanXclkSubNodeWidget->pos();
    QRect subNodeRect = m_clkKeyscanXclkSubNodeWidget->rect();
...
QPoint ClockConfigWidget::getClkWgnXclkSubNodeConnectionPoint() const
{
    if (!m_clkWgnXclkSubNodeWidget || !m_flowWidget) {
        return QPoint();
    }

    // 获取子节点widget在其父widget中的位置
    QPoint subNodePos = m_clkWgnXclkSubNodeWidget->pos();
    QRect subNodeRect = m_clkWgnXclkSubNodeWidget->rect();
...
QWidget* ClockConfigWidget::findClockWidgetByName(const QString& key, QString* resolvedName) const
{
    ...
    if (!w) w = tryFind(m_clkAPBI2CSubNodeWidgets);
    if (!w) w = tryFind(m_clkAPBVCSYSSubNodeWidgets);
    if (!w) w = tryFind(m_clkX2PSubNodeWidgets);
...
QWidget* ClockConfigWidget::getWidgetAt(const QPoint& pos)
{
    ...
    allWidgets << m_inputWidget << m_pllWidget << m_subPllWidget << m_outputWidget
               ...
               << m_clkFPLLSubNodeWidget << m_clkTPLLSubNodeWidget << m_clkMPLLSubNodeWidget
               << m_clkFAB100MSubNodeWidget << m_clkXtalMiscSubNodeWidget << m_clkI2CSubNodeWidget
               << m_clkAPBI2CSubNodeWidget << m_clkAPBVCSYSSubNodeWidget << m_clkX2PSubNodeWidget
               << m_clkRTCSYSSubNodeWidget<< m_clkHSPeriSubNodeWidget << m_clkVIPSYS0SubNodeWidget
               << m_clkVIPSYS1SubNodeWidget << m_clkVIPSYS2SubNodeWidget << m_clkVIPSYS3SubNodeWidget << m_clkSPISubNodeWidget
               << m_clkKeyscanXclkSubNodeWidget << m_clkWgnXclkSubNodeWidget;
...
void ClockConfigWidget::updateWidgetGeometry(QWidget* widget, const QRect& newGeometry)
{
    ...
    // 动态自适应：如果是 fab_100M 子节点区域，使用布局 sizeHint 重新设置最小高度
    if (widget == m_clkFAB100MSubNodeWidget && m_clkFAB100MSubNodeLayout) {
        int h = m_clkFAB100MSubNodeLayout->sizeHint().height() + 20;
        if (h > m_clkFAB100MSubNodeWidget->minimumHeight()) {
            m_clkFAB100MSubNodeWidget->setMinimumHeight(h);
            ensureFlowCanvasFitsContent();
        }
    }
}
...
QString ClockConfigWidget::getWidgetModuleName(QWidget* widget)
{
    ...
    if (widget == m_clkAPBI2CSubNodeWidget) return "clk_apb_i2c";
    if (widget == m_clkAPBVCSYSSubNodeWidget) return "clk_apb_vcsys";
    if (widget == m_clkX2PSubNodeWidget) return "clk_x2p";
...
QWidget* ClockConfigWidget::getWidgetByModuleName(const QString& moduleName)
{
    ...
    if (moduleName == "clk_apb_i2c") return m_clkAPBI2CSubNodeWidget;
    if (moduleName == "clk_apb_vcsys") return m_clkAPBVCSYSSubNodeWidget;
    if (moduleName == "clk_x2p") return m_clkX2PSubNodeWidget;
    ...
    if (moduleName == "clk_vip_sys_2") return m_clkVIPSYS2SubNodeWidget;
    if (moduleName == "clk_vip_sys_3") return m_clkVIPSYS3SubNodeWidget;
    if (moduleName == "clk_spi") return m_clkSPISubNodeWidget;
    if (moduleName == "clk_keyscan_xclk") return m_clkKeyscanXclkSubNodeWidget;
    if (moduleName == "clk_wgn_xclk") return m_clkWgnXclkSubNodeWidget;
